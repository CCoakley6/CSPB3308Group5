...
def playCard(data):
    ...
    if len(players) == 2:
        if players[0]["token"] == 7:
            ...
            connection.commit()
            socketio.emit("message", { "message": "Congratulations, {} Win the Game".format(players[0]["name"])}, room = roomID)
            return
            ...
            connection.commit()
            socketio.emit("message", { "message": "Congratulations, {} Win the Game".format(players[1]["name"])}, room = roomID)
            return
        elif players[0]['outOfRound'] == True:
            socketio.emit("message", { "message": "Congratulations, {} Win the Round".format(players[1]["name"]), "players": players}, room = roomID)
            return
        elif players[1]["outOfRound"] == True:
            socketio.emit("message", { "message": "Congratulations, {} Win the Round".format(players[0]["name"]), "players": players}, room = roomID)
            return
    ...
    if len(players) == 3:
        ...
        if len(winner) == 1:
            if winner[0]["token"] == 5:
                ...
                connection.commit()
                socketio.emit("message", { "message": "Congratulations, {} Win the Game".format(winner[0]["name"])}, room = roomID)
                return
        if len(victor) == 1:
            socketio.emit("message", { "message": "Congratulations, {} Win the Round".format(victor[0]["name"]), "players": players}, room = roomID)
            return
